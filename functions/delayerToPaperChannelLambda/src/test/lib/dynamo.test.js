const { getItems, deleteItems } = require("../../app/lib/dynamo");
...
describe("getItems", () => {
    ...
    it("should return items from the table", async () => {
        ...
        dynamoDocumentClientMock.onCall(0).resolves({ Items: items });

        const result = await getItems(deliveryDate);
    ...
    it("should return an empty array if no items are found", async () => {
        const deliveryDate = "2023-01-01";
        dynamoDocumentClientMock.onCall(0).resolves({ Items: [] });

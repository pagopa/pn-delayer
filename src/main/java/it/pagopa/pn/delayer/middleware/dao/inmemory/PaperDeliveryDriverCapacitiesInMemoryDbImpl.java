    ...
    public Mono<Integer> getPaperDeliveryDriverCapacities(String tenderId, String deliveryDriverId, String geoKey, Instant deliveryDate) {
        return Mono.just(data.values()
                ...
                .map(paperDeliveryDriverCapacity -> {
                    log.info("found PaperDeliveryDriverCapacities with deliveryDriverId: {}, geoKey: {}, tenderId: {}, deliveryDate: {}",
                            deliveryDriverId, geoKey, tenderId, deliveryDate);
                    return paperDeliveryDriverCapacity.getCapacity();
                })
                .orElseGet(() -> {
                    log.info("PaperDeliveryDriverCapacities not found with deliveryDriverId: {}, geoKey: {}, tenderId: {}, deliveryDate: {} returning 0",
                            deliveryDriverId, geoKey, tenderId, deliveryDate);
                    return 0;
                }));
    }

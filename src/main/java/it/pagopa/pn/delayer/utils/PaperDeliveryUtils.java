...
public class PaperDeliveryUtils {
    ...
    public List<PaperDeliveryReadyToSend> mapToPaperDeliveryReadyToSend(List<PaperDeliveryHighPriority> items, Integer capCapacity, Integer dispatchedCapCapacity) {
        List<PaperDeliveryReadyToSend> paperDeliveryReadyToSendList = items.stream()
                .map(paperDeliveryHighPriority -> PaperDeliveryReadyToSend.builder()
                        .requestId(paperDeliveryHighPriority.getRequestId())
                        .iun(paperDeliveryHighPriority.getIun())
                        .deliveryDriverId(paperDeliveryHighPriority.getDeliveryDriverId())
                        .province(paperDeliveryHighPriority.getProvince())
                        .build())
                .toList();

...
class EvaluateResidualCapacityJobServiceTest {
    ...
    private EvaluateResidualCapacityJobServiceImpl evaluateResidualCapacityJob;
    ...
    void setUp() {
        ...
        evaluateResidualCapacityJob = new EvaluateResidualCapacityJobServiceImpl(paperDeliveryUtils);
    }
    ...
    void startEvaluateResidualCapacityJob_NoProvinceCapacity() {
        ...
        verify(paperDeliveryCounterDAO, times(1)).updateExcludeCounter(eq(deliveryWeek), anyMap());
    }
    ...
    void startEvaluateResidualCapacityJob_NoCapCapacityForOneCap_WithLastEvaluatedKey() {
        ...
        when(paperDeliveryDAO.retrievePaperDeliveries(eq(WorkflowStepEnum.EVALUATE_RESIDUAL_CAPACITY), any(), eq(String.join("~", unifiedDeliveryDriver, province)), any(), eq(4), any()))
    ...
    void startEvaluateResidualCapacityJob_NoCapCapacityForOneCap_WithLastEvaluatedKey_WithoutProvinceCapacity() {
        ...
        when(paperDeliveryDAO.retrievePaperDeliveries(eq(WorkflowStepEnum.EVALUATE_RESIDUAL_CAPACITY), any(), eq(String.join("~", unifiedDeliveryDriver, province)), any(), eq(3), any()))
                .thenReturn(Mono.just(page));

        when(paperDeliveryDAO.retrievePaperDeliveries(eq(WorkflowStepEnum.EVALUATE_RESIDUAL_CAPACITY), any(), eq(String.join("~", unifiedDeliveryDriver, province)), any(), eq(2), any()))
